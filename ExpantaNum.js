...
;(function (globalScope) {
  ...
  P.softcap = function (start, power) {
    var number
    number = this.clone()
    start = ExpantaNum(start)
    power = ExpantaNum(power)
    if (number.gt(start)) {
    number = number.div(start).pow(power).mul(start)
  }
    return number;
  };
  P.overflow = function (start, power){
    var number
   number = this.clone()
   start = E(start)
    if (number.gt(start)) {
      let s = start.log10()
      number = E(10).pow(number.log10().div(s).pow(power).mul(s))
  }
	return number;
  };
  P.siltation = function (start, power){
    var number
   number = this.clone()
   start = E(start)
    if (number.gt(start)) {
      let s = start.slog()
      number = E(10).tetr(number.slog().div(s).tetr(power).mul(s))
  }
	return number;
  };
  P.warp = function (start, power){
    var number
   number = this.clone()
   start = E(start)
    if (number.gt(start)) {
      let s = start.mlog()
      number = E(10).pent(number.mlog().div(s).pent(power).mul(s))
  }
	return number;
  };
